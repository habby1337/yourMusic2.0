...
const GenreCard = ({ title, description, imageUrl }: { title: string; description: string; imageUrl: string }) => {
	return (
		<Card className="w-3/6 h-56 relative overflow-clip border-0 bg-neutral-800 rounded-3xl flex-none mb-8">
			<div className="absolute flex radial-bg  backdrop-blur-[1px] text-white w-full h-48 font-bold justify-center items-center">
				<div className="text-lg   ">{title}</div>
			</div>
			<img src={imageUrl} alt="" className="w-full h-48" />
